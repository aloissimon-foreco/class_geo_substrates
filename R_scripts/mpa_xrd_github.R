...
setwd("~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/PET_RDA_rechnung")
#establish driver connection to database
mpa_xrd <- odbcConnect("mpa_xrd_github.mdb")
...
#ff: proportion of the fine fraction (<2 mm) of the total sample (<32 mm) [%].
...
#triangle XRD without CM weight
...
#CM weighted by the fine fraction (Feinanteil)
#CM_w= CM_comp* (1+ ff???100) (XRD)
...
#components_w= CM_w  - CM_comp + ???MCG_comp + M_dc
...
#save the table eingRDA in the database (optional)
...
#creation data table mpa+xrd-3eck with columns "samples", "triangle assigned to XRD with weighted CM" and "groups"
...
#mode of the components weighted by the mass fractions (MPA)
#MPA_K
for (i in ProbeNR){
...
#sum of the mode of the components weighted by the mass fractions (MPA)
...
#triangle assigned MPA with CM weighted by the fine fraction
MPA_XRD_3eck$MPA <- ifelse((MPA_XRD_3eck$K_MPA + MPA_XRD_3eck$D_MPA)/MPA_XRD_3eck$sumalleTeile_CM> 0.1, "D-2", "D-3" )
MPA_XRD_3eck$MPA <- ifelse((MPA_XRD_3eck$K_MPA + MPA_XRD_3eck$D_MPA)/MPA_XRD_3eck$sumalleTeile_CM> 0.75, "D-1", MPA_XRD_3eck$MPA)

#add XRD MCG proportions to same data frame MPA_XRD_3eck
...
#dataframe of samples without MPA data
XRD_3eck <- MPA_XRD_3eck[which(is.na(MPA_XRD_3eck$MPA) & is.na(MPA_XRD_3eck$summeTeile)),]

#grains size fraction proportion [%] weighted by surface factors: gsf_proportion_i * sf_i
...
#weighted fine fraction proportion ff_w= SAf_w+cSIf_w+ mSIf_w+ fSIf_w+ CLf_w
...
#save the table MPA_XRD_3eck in the database (optional)
...
D1 <- D1[complete.cases(D1$K_dr1),]
D2 <- D2[complete.cases(D2$K_D_dr2),]
D3 <- D3[complete.cases(D3$MM_dr3),]


#plot triangle1
for (i in D1$ProbeNR){
load(file="~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/dreieck1.rda")#add directory path to file
TT.set(reset=TRUE)
TT.add("tas.TT"= tas)
...
setwd("~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/PET_RDA_rechnung/dreiecke_PNG")#add directory path to folder
...
#triangle 2
for (i in D2$ProbeNR){
load(file="~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/dreieck2.rda")#add directory path to file
TT.set(reset=TRUE)
TT.add("tat.TT"= tat)
...
setwd("~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/PET_RDA_rechnung/dreiecke_PNG")#add directory path
png(file=paste0(i,".png"), width=1500,height=1500, res=140, pointsize=8)
...
#plot triangle3
for (i in D3$ProbeNR){
  load(file="~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/dreieck3.rda")#add directory path to file
  TT.set(reset=TRUE)
  TT.add("tap.TT"= tap)
...
#plot triangle1 - samples with no MPA

D1noMPA <- XRD_3eck[XRD_3eck$XRD %in% "D-1",]
for (i in D1noMPA$ProbeNR){
  load(file="~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/dreieck1.rda")#add directory path to file
  TT.set(reset=TRUE)
  TT.add("tas.TT"= tas)
  ...
  XRD <- data.frame(
    "z"=1,
    "Dolomit" = D1noMPA[D1noMPA$ProbeNR %in% i, "D_dr1_XRD"],
    "TM_Silikate" = D1noMPA[D1noMPA$ProbeNR %in% i, "FM_MM_CM_dr1_XRD"],
    "Kalk" = D1noMPA[D1noMPA$ProbeNR %in% i, "K_dr1_XRD"])
  ...
  TT.points(tri.data = XRD, geo= D1T, css.names=c("Dolomit","TM_Silikate","Kalk"), #z.name="z",
            cex= c(6),
            pch=c(17))
  
  legend("topright", c("XRD"),
         pt.cex = c(4),
         xjust=4, yjust=1,
         cex=4,
...
#triangle 2 - samples with no MPA

D2noMPA <- XRD_3eck[XRD_3eck$XRD %in% "D-2",]

for (i in D2noMPA$ProbeNR){
  load(file="~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/dreieck2.rda")#add directory path to file
  TT.set(reset=TRUE)
  TT.add("tat.TT"= tat)
  ...
  XRD <- data.frame(
    "z"=1,
    "Karbonat" = D2noMPA[D2noMPA$ProbeNR %in% i, "K_D_dr2_XRD"],
    "TM_BS" = D2noMPA[D2noMPA$ProbeNR %in% i, "CM_MM_dr2_XRD"],
    "SS" = D2noMPA[D2noMPA$ProbeNR %in% i, "FM_dr2_XRD"])
  ...
  TT.points(tri.data = XRD, geo= D1T, css.names=c("Karbonat","TM_BS","SS"), #z.name="z",
            cex= c(6),
            pch=c(17))
...
#plot triangle3 - samples with no MPA
D3noMPA <- XRD_3eck[XRD_3eck$XRD %in% "D-3",]

for (i in D3noMPA$ProbeNR){
  load(file="~/Links4Soils_24032020/T3_case_study_biomass/substrat_deck/dreieck3.rda")#add directory path to file
  TT.set(reset=TRUE)
  TT.add("tap.TT"= tap)
  ...
  XRD <- data.frame(
    "z"=1,
    "QF_Mu" = D3noMPA[D3noMPA$ProbeNR %in% i, "FM_dr3_XRD"],
    "TM" = D3noMPA[D3noMPA$ProbeNR %in% i, "CM_dr3_XRD"],
    "BS" = D3noMPA[D3noMPA$ProbeNR %in% i, "MM_dr3_XRD"])
  ...
  TT.points(tri.data = XRD, geo= D1T, css.names=c("QF_Mu","TM","BS"), #z.name="z",
            cex= c(6),
            pch=c(17))
...
DW <- DW[!DW$ProbeNR %in% XRD_3eck$ProbeNR,]
DW <- DW[complete.cases(DW$ff_w),]
